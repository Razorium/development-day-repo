...
const LoginPage = () => {
  ...
  const validate = () => {
    let tempErrors = {};
    if (!formData.familyName) tempErrors.familyName = "Family name is required.";
    if (!formData.givenName) tempErrors.givenName = "Given name is required.";
    if (!formData.email) tempErrors.email = "Email is required.";
    else if (!/\S+@\S+\.\S+/.test(formData.email)) tempErrors.email = "Email is invalid.";
    if (!formData.age) tempErrors.age = "Age is required.";
    if (!formData.country) tempErrors.country = "Country is required.";
    if (formData.country === 'Other' && !formData.otherCountry) tempErrors.otherCountry = "Please specify your country.";
      ...
      <div className="p-8 bg-white rounded-lg shadow-lg max-w-md w-full space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {formData.country === 'Other' && (
            <input type="text" name="otherCountry" placeholder="Please specify your country"
                   value={formData.otherCountry} onChange={handleChange}
                   className="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800" />
          )}
